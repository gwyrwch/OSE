...
void init(HWND hWnd) {
    ...
    drawCheckbox = CreateWindow(
        L"button", L"draw", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX,
        55, 170, 90, 30, hWnd,
        (HMENU)DRAW, hInst, NULL);
...
void sendMsg(HWND hWnd) {
    int check = SendMessage(drawCheckbox, BM_GETCHECK, 0, 0);

    if (color != -1 && figureType != -1 && check == BST_CHECKED) {
        int param = color * 1000 + figureType;
        PostMessage(HWND_BROADCAST, WM_DRAW_FIGURE, param, 0);
    }
}
